import Github from "./github";
import LinkedIn from "./linkedin";
import Chevron from "./chevron";
import Mail from "./mail";
import Suitcase from "./suitcase";
import EduHat from "./edu-hat";
import CodeSheet from "./code-sheet";
import Person from "./person";
import Docker from "./tech-icons/docker";
import Javascript from "./tech-icons/javascript";
import NextJS from "./tech-icons/nextjs";
import React from "./tech-icons/react";
import Symfony from "./tech-icons/symfony";
import Unreal from "./tech-icons/unreal";
import CPlusPlus from "./tech-icons/cplusplus";
import VueJs from "./tech-icons/vuejs";
import Figma from "./tech-icons/figma";
import Typescript from "./tech-icons/typescript";
import Gatsby from "./tech-icons/gatsby";
import Php from "./tech-icons/php";
import Laravel from "./tech-icons/laravel";
import Java from "./tech-icons/java";
import Spring from "./tech-icons/spring";
import Maven from "./tech-icons/maven";

export {
  Github,
  LinkedIn,
  Chevron,
  Mail,
  Suitcase,
  EduHat,
  CodeSheet,
  Person,
  Javascript,
  NextJS,
  React,
  Symfony,
  Docker,
  Unreal,
  CPlusPlus,
  VueJs,
  Figma,
  Typescript,
  Gatsby,
  Php,
  Laravel,
  Java,
  Spring,
  Maven,
};
